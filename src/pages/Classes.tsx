...
const classes = [
  { id: 1, name: 'การออกแบบอัลกอริทึม', code: 'CS301', section: 'เซกชัน 1', instructor: 'อ.ดร. อลัน ทัวริง', color: 'bg-blue-600', assignments: 3 },
  { id: 2, name: 'โครงสร้างข้อมูลพื้นฐาน', code: 'CS201', section: 'เซกชัน 2', instructor: 'ศ. เอดา เลิฟเลซ', color: 'bg-indigo-600', assignments: 1 },
  { id: 3, name: 'การเขียนโปรแกรมเพื่อการแข่งขัน', code: 'CP101', section: 'ค่าย สอวน. คอมพิวเตอร์ ค่าย 1', instructor: 'โค้ชประจำศูนย์', color: 'bg-emerald-600', assignments: 5 },
];
  ...
  return (
    <div className="space-y-6">
      <div className="sm:flex sm:items-center sm:justify-between border-b border-gray-200 dark:border-zinc-800 pb-5">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-gray-100">ชั้นเรียน</h1>
          <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">หลักสูตรการเรียนการสอนที่คุณกำลังลงทะเบียนเข้าร่วมอยู่ในระบบ</p>
        </div>
        ...
        {classes.map((cls) => (
          <div key={cls.id} className="group relative flex flex-col overflow-hidden rounded-xl border border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-sm transition-all hover:shadow-md cursor-pointer">
            <div className={`${cls.color} h-24 p-4 text-white flex justify-between items-start`}>
              ...
              <button className="p-1 hover:bg-black/10 rounded-full transition-colors" aria-label="ตัวเลือกเพิ่มเติม">
                <MoreVertical className="h-5 w-5" />
              </button>
            </div>
            
            <div className="p-4 flex-1">
              <p className="text-sm text-gray-500 dark:text-gray-400">ผู้สอน: {cls.instructor}</p>
              <div className="mt-4 flex gap-2">
                <span className="inline-flex items-center gap-1.5 rounded-full px-2 py-1 text-xs font-medium bg-gray-100 dark:bg-zinc-800 text-gray-600 dark:text-gray-300">
                  ...
                  {cls.assignments} งานที่ยังไม่ส่ง
                </span>
              </div>
            </div>
            
            <div className="border-t border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-800/50 px-4 py-3 text-right">
              <span className="text-sm font-medium text-blue-600 dark:text-blue-500 hover:text-blue-500 dark:hover:text-blue-400">ดูรายการใบงาน &rarr;</span>
            </div>
          </div>
        ))}
      </div>
